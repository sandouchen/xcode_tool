...
- (instancetype)init {
	if(self = [super init]) {
		...
		self.string_GB = [NSString stringWithContentsOfFile:[resourcePath stringByAppendingPathComponent:@"gb.txt"] encoding:NSUTF8StringEncoding error:&error];
        
		self.string_BIG5 = [NSString stringWithContentsOfFile:[resourcePath stringByAppendingPathComponent:@"big5.txt"] encoding:NSUTF8StringEncoding error:&error];
	}
...
- (NSString *)gbToBig5:(NSString *)srcString {
	...
	for (NSInteger i = 0; i < length; i++) {
		...
		if(gbRange.location != NSNotFound) {
			...
			srcString = [srcString stringByReplacingCharactersInRange:NSMakeRange(i, 1) withString:big5String];
            }
	}
...
- (NSString *)big5ToGb:(NSString *)srcString {
	...
	for (NSInteger i = 0; i< length; i++) {
		...
		if(big5Range.location != NSNotFound) {
			...
			srcString = [srcString stringByReplacingCharactersInRange:NSMakeRange(i, 1) withString:gbString];
		}
	}
