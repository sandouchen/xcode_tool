    ...
    [SDNetworkHelper networkStatusWithBlock:^(SDNetworkStatusType status) {
        switch (status) {
            case SDNetworkStatusUnknown:
                [self.window.rootViewController.view makeToast:NSLocalizedString(@"未知名网络,请注意!!!", nil) duration:2 position:CSToastPositionCenter];
                break;
                
            case SDNetworkStatusNotReachable:
                [self.window.rootViewController.view makeToast:NSLocalizedString(@"断网了,请注意!!!", nil) duration:2 position:CSToastPositionCenter];
                break;
                
            case SDNetworkStatusReachableViaWiFi:
                [self.window.rootViewController.view makeToast:NSLocalizedString(@"已连接WIFI,可以使劲用!!!", nil) duration:2 position:CSToastPositionCenter];
                break;
                
            case SDNetworkStatusReachableViaWWAN:
                [self.window.rootViewController.view makeToast:NSLocalizedString(@"手机4G网络,省着点用!!!", nil) duration:2 position:CSToastPositionCenter];
                break;
