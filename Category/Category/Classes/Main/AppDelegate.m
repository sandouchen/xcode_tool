...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    SDNewFeatureViewController *newFeature = [[SDNewFeatureViewController alloc] init];
    
    [self.window chooseFirstStartViewController:newFeature orSecondStartViewController:vc];
    ...
    [SDNetworkHelper networkStatusWithBlock:^(SDNetworkStatusType status) {
        switch (status) {
            case SDNetworkStatusUnknown:
                [MBProgressHUD showWarn:NSLocalizedString(@"未知名网络,请注意!!!", nil) toView:nil];
                break;
                
            case SDNetworkStatusNotReachable:
                [MBProgressHUD showError:NSLocalizedString(@"断网了,请注意!!!", nil) toView:nil];
                break;
                
            case SDNetworkStatusReachableViaWiFi:
                [MBProgressHUD showInfo:NSLocalizedString(@"已连接WIFI,可以使劲用!!!", nil) toView:nil];
                break;
                
            case SDNetworkStatusReachableViaWWAN:
                [MBProgressHUD showWarn:NSLocalizedString(@"手机4G网络,省着点用!!!", nil) toView:nil];
                break;
